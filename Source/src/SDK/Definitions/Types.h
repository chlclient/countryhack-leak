...
struct Color_t
{
	...
	// ==================== NEW RAINBOW FUNCTION ====================
	/**
	 * Returns a vibrant rainbow color based on time/position.
	 * Perfect for animated UI, ESP, text, etc.
	 *
	 * @param flTime   Usually current time in seconds (or any increasing value)
	 * @param flSpeed  How fast the rainbow cycles (default = 1.0)
	 * @param flSat    Saturation (0-100)
	 * @param flVal    Value/Brightness (0-100)
	 * @return Rainbow Color_t
	 */
	static inline Color_t Rainbow(float flTime, float flSpeed = 1.0f, float flSat = 100.f, float flVal = 100.f, byte alpha = 255)
	{
		float flHue = std::fmod(flTime * flSpeed, 360.0f);
		Color_t col;
		col.SetHSV(flHue, flSat, flVal, alpha);
		return col;
	}
};
