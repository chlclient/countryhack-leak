...
void CDraw::DrawCorneredBox(int x, int y, int w, int h, Color_t tColor, Color_t tColorOut)
{
	int length = std::min(w, h) / 5; // corner length, feel free to change

	// Top left
	H::Draw.Line(x, y, x + length, y, tColor);           // horizontal
	H::Draw.Line(x, y, x, y + length, tColor);           // vertical
	H::Draw.Line(x - 1, y - 1, x + length + 1, y - 1, tColorOut);
	H::Draw.Line(x - 1, y - 1, x - 1, y + length + 1, tColorOut);

	// Top right
	H::Draw.Line(x + w, y, x + w - length, y, tColor);
	H::Draw.Line(x + w, y, x + w, y + length, tColor);
	H::Draw.Line(x + w + 1, y - 1, x + w - length - 1, y - 1, tColorOut);
	H::Draw.Line(x + w + 1, y - 1, x + w + 1, y + length + 1, tColorOut);

	// Bottom left
	H::Draw.Line(x, y + h, x + length, y + h, tColor);
	H::Draw.Line(x, y + h, x, y + h - length, tColor);
	H::Draw.Line(x - 1, y + h + 1, x + length + 1, y + h + 1, tColorOut);
	H::Draw.Line(x - 1, y + h + 1, x - 1, y + h - length - 1, tColorOut);

	// Bottom right
	H::Draw.Line(x + w, y + h, x + w - length, y + h, tColor);
	H::Draw.Line(x + w, y + h, x + w, y + h - length, tColor);
	H::Draw.Line(x + w + 1, y + h + 1, x + w - length - 1, y + h + 1, tColorOut);
	H::Draw.Line(x + w + 1, y + h + 1, x + w + 1, y + h - length - 1, tColorOut);
}
...
void CDraw::GradientRect(int x, int y, int w, int h, Color_t tColorTop, Color_t tColorBottom, bool bHorizontal)
{
	...
	I::MatSystemSurface->DrawFilledRectFade(x, y, x + w, y + h, 255, 255, bHorizontal);

	I::MatSystemSurface->DrawSetColor(tColorBottom.r, tColorBottom.g, tColorBottom.b, tColorBottom.a);
	I::MatSystemSurface->DrawFilledRectFade(x, y, x + w, y + h, 0, 255, bHorizontal);
}
