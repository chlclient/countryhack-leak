...
NAMESPACE_BEGIN(Vars)
	NAMESPACE_BEGIN(Menu)
		CVar(CheatTitle, "Cheat title", std::string("Countryhack"), VISUAL | DROPDOWN_AUTOUPDATE);
		CVar(CheatTag, "Cheat tag", std::string("[Countryhack]"), VISUAL);
		...
		CVarEnum(Indicators, "Indicators", 0b00000, VISUAL | DROPDOWN_MULTI, nullptr,
			VA_LIST("Ticks", "Crit hack", "Spectators", "Conditions", "Features", "Seed prediction", "Spotify"),
			Ticks = 1 << 0, CritHack = 1 << 1, Spectators = 1 << 2, Conditions = 1 << 3, Features = 1 << 4, SeedPrediction = 1 << 5, Spotify = 1 << 6);
		...
		CVar(IndicatorsDisplay, "Indicators display", DragBox_t(), VISUAL | NOBIND);
		CVar(SpotifyDisplay, "Spotify display", DragBox_t(), VISUAL | NOBIND);
		...
		NAMESPACE_BEGIN(Theme)
			CVar(Accent, "Accent", Color_t(175, 150, 255, 255), VISUAL);
		   CVar(Accent2, "Accent 2", Color_t(150, 100, 255, 255), VISUAL);
			CVar(Background, "Background", Color_t(0, 0, 0, 250), VISUAL);
			CVar(Active, "Active", Color_t(255, 255, 255, 255), VISUAL);
			CVar(Inactive, "Inactive", Color_t(150, 150, 150, 255), VISUAL);
		NAMESPACE_END(Theme)
	NAMESPACE_END(Menu)
	...
	NAMESPACE_BEGIN(Aimbot)
		NAMESPACE_BEGIN(General, Aimbot)
			...
			CVar(IgnoreInvisible, "Ignore invis", 50.f, SLIDER_CLAMP | SLIDER_PRECISION, 0.f, 100.f, 10.f, "%g%%");
			...
			CVar(AutoShoot, "Autoshoot", true);
			CVar(FOVCircle, "FOV circle", true, VISUAL);
			CVar(LeadAndRestrict, "Restrict aim", false, VISUAL);
			CVar(NoSpread, "Nospread", false);
		...
		NAMESPACE_BEGIN(Projectile)
			...
			CVarEnum(Modifiers, VA_LIST("Modifiers", "Projectile modifiers"), 0b0010, DROPDOWN_MULTI, nullptr,
				VA_LIST("Charge weapon", "Cancel charge", "Use arm time", "Air splash", "High arc", "Target dormant"),
			...
			CVar(Airstrikesavepost, "Airstrike save", false);
			CVar(Airstrikeclearpos, "Airstrike clear", false);
			CVar(Neckbreak, "Neckbreak", false);
			CVar(Neckbreakstep, "Neckbreak step", 4, SLIDER_CLAMP, 0, 21);
	...
	NAMESPACE_BEGIN(Doubletap)
		...
		CVar(WarpPulseSpeed, "Warp pulse speed", 0.5f, SLIDER_CLAMP | SLIDER_PRECISION, 0.f, 5.f, 0.05f);
		...
		CVar(WarpRateMin, "Warp rate min", 2, SLIDER_CLAMP, 1, 22);
	...
	NAMESPACE_BEGIN(Speedhack)
		CVar(Scale, VA_LIST("Scale", "SpeedHack scale"), 1, NONE, 0, 50);
	...
	NAMESPACE_BEGIN(AntiAim, Antiaim)
		...
		CVar(FlipRealFake, "Flip real/fake", false);
	...
	NAMESPACE_BEGIN(Visuals)
		...
		NAMESPACE_BEGIN(Trajectory)
			...
			NAMESPACE_END(ProjectileTrajectory)
				...
				CVar(Recorder, "Recorder", false);
				CVar(StartRecord, "Start record", false);
			CVar(PlayRecord, "Play record", false);
			CVar(ClearRecord, "Clear record", false);
		...
		NAMESPACE_BEGIN(DebugDraw)
			CVar(NoSpreadDraw, "Draw NoSpread", false, VISUAL);
			CVar(ResolverData, "Show Resolver Data", false, VISUAL);
			CVar(PacketJitter, "Show Packet Jitter", false, VISUAL);
		NAMESPACE_END(DebugDraw)
