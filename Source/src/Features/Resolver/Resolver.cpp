...
std::optional<Vec3> CResolver::GetAnglesForSniperDot(CTFPlayer* pEntity, CTFPlayerResource* pResource)
{
...
void CResolver::FrameStageNotify()
{
	...
	for (auto& pEntity : H::Entities.GetGroup(EntityEnum::PlayerAll))
	{
		...
		{
			if (auto vAngles = GetAnglesForSniperDot(pPlayer, pResource))
				tData.m_flPitch = vAngles->x;
...
void CResolver::Draw()
{
	// Debug drawing for resolver data can be added here if needed
}