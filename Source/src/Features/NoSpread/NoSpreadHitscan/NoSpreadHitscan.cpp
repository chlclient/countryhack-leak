...
bool CNoSpreadHitscan::ParsePlayerPerf(const std::string& sMsg)
{
	...
	if (tMatch.size() == 2)
	{
		...
		// Calculate Round Trip Time (RTT) and Jitter for playerperf packets
		double dCurrentRTT = SDK::PlatFloatTime() - m_dRequestTime;
		if (DebugJitter::g_dLastRTT > 0.0)
			DebugJitter::g_dJitter = std::abs(dCurrentRTT - DebugJitter::g_dLastRTT);
		DebugJitter::g_dLastRTT = dCurrentRTT;
		...
		{
			m_vTimeDeltas.push_back(m_flServerTime - m_dRequestTime);
...
void CNoSpreadHitscan::Draw(CTFPlayer* pLocal)
{
	...
	H::Draw.StringOutlined(fFont, x, y, cColor, Vars::Menu::Theme::Background.Value, align, std::format("Seed {}", m_iSeed).c_str());
	H::Draw.StringOutlined(fFont, x, y += nTall, cColor, Vars::Menu::Theme::Background.Value, align, std::format("Uptime {}", GetFormat(m_flServerTime)).c_str());
	...
	// Renders Jitter and Packet Latency (RTT) in milliseconds underneath
	H::Draw.StringOutlined(fFont, x, y += nTall, cColor, Vars::Menu::Theme::Background.Value, align, 
		std::format("Jitter {:.2f}ms (RTT {:.1f}ms)", DebugJitter::g_dJitter * 1000.0, DebugJitter::g_dLastRTT * 1000.0).c_str());
}