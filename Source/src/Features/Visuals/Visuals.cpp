...
void CVisuals::DrawFeatureWindow(CTFPlayer* pLocal)
{
	if(!(Vars::Menu::Indicators.Value & Vars::Menu::IndicatorsEnum::Features))
		return;
	...
	static float flFakeLatency = 0.f;
	{
		static Timer tTimer = {};
		if (tTimer.Run(0.5f))
			flFakeLatency = F::Backtrack.GetFakeLatency();
	}
	float flFakeLerp = F::Backtrack.GetFakeInterp() > G::Lerp ? F::Backtrack.GetFakeInterp() : 0.f;

	float flFake = std::min(flFakeLatency + flFakeLerp, F::Backtrack.m_flMaxUnlag) * 1000;
	float flLatency = std::max(pNetChan->GetLatency(FLOW_INCOMING) + pNetChan->GetLatency(FLOW_OUTGOING) - flFakeLatency, 0.f) * 1000;
	auto cmds = I::ClientState->chokedcommands + 1;
	const DragBox_t dtPos = Vars::Menu::IndicatorsDisplay.Value;
	const int windowX = dtPos.x;
	const int windowY = dtPos.y;
	const int windowWidth = H::Draw.Scale(180, Scale_Round);
	const int windowHeight = H::Draw.Scale(180, Scale_Round);
	const int headerHeight = H::Draw.Scale(20, Scale_Round);

	const Color_t colBackground = {25,25,25,240};
	const Color_t colBorder = Color_t{ 0, 0, 0, 255 };
	const Color_t colHeader = Vars::Menu::Theme::Accent.Value;
	const Color_t colText = F::NoSpreadHitscan.m_bSynced ? Vars::Menu::Theme::Active.Value : Vars::Menu::Theme::Inactive.Value;

	H::Draw.FillRect(windowX, windowY, windowWidth, windowHeight, colBackground);
	H::Draw.LineRect(windowX, windowY, windowWidth, headerHeight, colBorder);
	H::Draw.LineRect(windowX, windowY, windowWidth, windowHeight, colBorder);

	const auto& fFont = H::Fonts.GetFont(FONT_INDICATORS);
	const int textPaddingX = H::Draw.Scale(6, Scale_Round);
	const int textPaddingY = H::Draw.Scale(4, Scale_Round);

	H::Draw.StringOutlined(
		fFont,
		windowX + textPaddingX,
		windowY + textPaddingY,
		colText,
		colBackground,
		ALIGN_TOPLEFT,
		"INFORMATION PANEL"
	...
	int currentY = windowY + headerHeight + textPaddingY;
	const int lineSpacing = fFont.m_nTall + H::Draw.Scale(2, Scale_Round);

	float spreadzingtons = F::NoSpreadHitscan.Spread;
	constexpr float flSpreadRMS = 0.5774f;
	const float flAvgSpread = spreadzingtons * flSpreadRMS;
	...
	H::Draw.StringOutlined(
		...
		colBackground,
		ALIGN_TOPLEFT,
		std::format("SPREAD: {:.3f}%", flAvgSpread).c_str()
	);
	...
	H::Draw.StringOutlined(
		...
		colBackground,
		ALIGN_TOPLEFT,
		std::format("PING: {:.0f}ms", flLatency).c_str()
	);
	currentY += lineSpacing;
	...
	H::Draw.StringOutlined(
		...
		colBackground,
		ALIGN_TOPLEFT,
		std::format("CHOKE: {}t", cmds).c_str()
	);
	H::Draw.StringOutlined(
		fFont, windowX + textPaddingX,
		currentY + lineSpacing,
		colText,
		colBackground,
		ALIGN_TOPLEFT,
		std::format("SPEED: {:.2f}us", vVelocity.Length()).c_str()
	);
	...
	H::Draw.StringOutlined(
		...
		colBackground,
		ALIGN_TOPLEFT,
		std::format("FAKE: {:.0f}ms", flFake).c_str()
	);
...
void CVisuals::DrawPickupTimers()
{
	...
	for (auto it = m_vPickups.begin(); it != m_vPickups.end();)
	{
		...
		if (SDK::W2S(tPickup.m_vLocation, vScreen))
			H::Draw.StringOutlined(H::Fonts.GetFont(FONT_ESP), vScreen.x, vScreen.y, pGroup->m_tColor, Vars::Menu::Theme::Background.Value, ALIGN_CENTER, std::format("t:{:.1f}s", flTime).c_str());

		it++;
	}
}

std::string GetSpotifyTitle()
{
	// Find Spotify.exe process
	DWORD dwSpotifyPID = 0;
	...
	// Find the main window belonging to that PID
	...
	auto EnumWindowsCallback = [](HWND hwnd, LPARAM lParam) -> BOOL
		{
			...
			{
				// Only want visible windows with a title
				if (IsWindowVisible(hwnd) && GetWindowTextLengthW(hwnd) > 0)
				{
					*reinterpret_cast<HWND*>(lParam + sizeof(DWORD)) = hwnd; // hacky but works
					return FALSE; // stop
				}
			}
			return TRUE;
	...
	EnumData data{ dwSpotifyPID, nullptr };
	...
	EnumWindows([](HWND hwnd, LPARAM lParam) -> BOOL
		{
			...
			if (pid == pData->pid && IsWindowVisible(hwnd) && GetWindowTextLengthW(hwnd) > 0)
			{
				pData->hwnd = hwnd;
				return FALSE;
			}
	...
	wchar_t szTitle[512]{};
	GetWindowTextW(data.hwnd, szTitle, 512);
	...
	char szBuffer[512]{};
	WideCharToMultiByte(CP_UTF8, 0, szTitle, -1, szBuffer, sizeof(szBuffer), nullptr, nullptr);

	std::string sTitle = szBuffer;

	// Remove " - Spotify" from the end if present
	const std::string sSuffix = "";
	if (sTitle.size() > sSuffix.size() && sTitle.ends_with(sSuffix))
		sTitle.erase(sTitle.size() - sSuffix.size());

	return sTitle.empty() ? "Idle" : sTitle;
}

void CVisuals::DrawSpotify(CTFPlayer* pLocal)
{
	if (!pLocal || !pLocal->IsAlive())
		return;

	const auto& fFont = H::Fonts.GetFont(FONT_INDICATORS);
	int x = Vars::Menu::SpotifyDisplay.Value.x, y = Vars::Menu::SpotifyDisplay.Value.y;
		static std::string sLastTitle = "Idle";
		static float flLastUpdate = 0.f;

		if (I::GlobalVars->curtime - flLastUpdate > 0.75f) // update every 0.75s
		{
			sLastTitle = GetSpotifyTitle();
			flLastUpdate = I::GlobalVars->curtime;
		}

		H::Draw.StringOutlined(fFont, x, y, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_TOPLEFT,
			std::format("Spotify: {}", sLastTitle).c_str());
}
...
void CVisuals::DrawDebugInfo(CTFPlayer* pLocal)
{
	...
	if (Vars::Debug::Info.Value)
	{
		...
		H::Draw.StringOutlined(fFont, x, y += nTall * 2, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_TOPLEFT, std::format("PosXyz: ({:.3f}, {:.3f}, {:.3f})", vOrigin.x, vOrigin.y, vOrigin.z).c_str());
		Vec3 vVelocity = pLocal->m_vecVelocity();
		...
		H::Draw.StringOutlined(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_TOPLEFT, std::format("Choke: {}, {}", G::Choking, I::ClientState->chokedcommands).c_str());
			H::Draw.StringOutlined(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_TOPLEFT, std::format("Lerp: {:.3f}", G::Lerp).c_str());
			
			// Warp prediction info
			if (F::Ticks.m_bWarp)
			{
				const auto& cWarpColor = Vars::Menu::Theme::Active.Value;
				H::Draw.StringOutlined(fFont, x, y += nTall * 2, cWarpColor, Vars::Menu::Theme::Background.Value, ALIGN_TOPLEFT, std::format("Warp Prediction: Active").c_str());
				H::Draw.StringOutlined(fFont, x, y += nTall, cWarpColor, Vars::Menu::Theme::Background.Value, ALIGN_TOPLEFT, std::format("Warp Rate: {}", Vars::Doubletap::WarpRate.Value).c_str());
				H::Draw.StringOutlined(fFont, x, y += nTall, cWarpColor, Vars::Menu::Theme::Background.Value, ALIGN_TOPLEFT, std::format("Shifted Ticks: {}", F::Ticks.m_iShiftedTicks).c_str());
			}
...
void CVisuals::Draw_Ticks(CTFPlayer* pLocal)
{
	...
	int iTicks = std::clamp(F::Ticks.m_iShiftedTicks + std::max(I::ClientState->chokedcommands - iAntiAimTicks, 0), 0, F::Ticks.m_iMaxUsrCmdProcessTicks);
	int iMax = std::max(Vars::Doubletap::RechargeLimit.Value - iAntiAimTicks, 0);

	float flRatio = iMax > 0 ? float(iTicks) / float(iMax) : 0.f;

	static float flLerpedRatio = 0.f;
	flLerpedRatio += (flRatio - flLerpedRatio) * 0.075f;

	int iSizeX = H::Draw.Scale(130, Scale_Round);
	int iSizeY = H::Draw.Scale(9, Scale_Round);
	int iPosX = dtPos.x - iSizeX / 2;
	int iPosY = dtPos.y + fFont.m_nTall;

	// Text
	int iTextY = dtPos.y + 4;

	const char* szShift = std::format("{}/{}", iTicks, iMax).c_str();
	H::Draw.StringOutlined(fFont, iPosX, iTextY, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_LEFT, szShift);

	std::string szStatus;
	 if (iTicks < 0 && !F::Ticks.m_bRecharge && !F::Ticks.m_iWait)
		szStatus = "NOT READY";
	else if (F::Ticks.m_bRecharge)
		szStatus = "CHARGING";
	...
	H::Draw.StringOutlined(fFont, iPosX + iSizeX, iTextY, Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value, ALIGN_RIGHT, szStatus.c_str());

	// Background bar
	H::Draw.FillRect(iPosX, iPosY, iSizeX, iSizeY, {14,14,14,230});

	// === Fade gradient based on tick ratio ===
	Color_t col1 = Vars::Menu::Theme::Accent.Value;
	Color_t col2 = Vars::Menu::Theme::Accent2.Value;

	col1.a = static_cast<byte>(col1.a * flLerpedRatio);
	col2.a = static_cast<byte>(col2.a * flLerpedRatio);

	H::Draw.GradientRect(iPosX, iPosY, iSizeX * flLerpedRatio, iSizeY, col1, col2, true);
