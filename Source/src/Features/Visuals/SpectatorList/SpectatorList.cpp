...
void CSpectatorList::Draw(CTFPlayer* pLocal)
{
	...
	const int nPadding = H::Draw.Scale(4, Scale_Round);
	...
	const int nBoxHeight = nTall + (static_cast<int>(m_vSpectators.size()) * nTall) + (nPadding * 2);
	const int nBoxWidth = H::Draw.Scale(200, Scale_Round);
	int nBoxX = x;
	int nBoxY = y - nPadding;

	// Adjust box position based on alignment
	if (align == ALIGN_TOPLEFT)
		nBoxX = x - nPadding;
	else if (align == ALIGN_TOPRIGHT)
		nBoxX = x - nBoxWidth + nPadding;
	else
		nBoxX = x - (nBoxWidth / 2);

	// Draw box background and outline
	H::Draw.FillRect(nBoxX, nBoxY, nBoxWidth, nBoxHeight, Vars::Menu::Theme::Background.Value);
	H::Draw.LineRect(nBoxX, nBoxY, nBoxWidth, nBoxHeight, Vars::Menu::Theme::Accent.Value);
	...
	for (auto& tSpectator : m_vSpectators)
	{
		...
		if (tSpectator.m_flRespawnIn != -1.f)
			H::Draw.StringOutlined(fFont, x + iconOffset, y, tColor, Vars::Menu::Theme::Background.Value, align, std::format("{} ({} - {}s)", tSpectator.m_sName, tSpectator.m_sMode, tSpectator.m_flRespawnIn).c_str());
		else
