...
static inline void StorePlayer(CTFPlayer* pPlayer, CTFPlayer* pLocal, Group_t* pGroup, std::unordered_map<CBaseEntity*, PlayerCache_t>& mCache)
{
	...
	tCache.m_bHeadCircle = pGroup->m_iESP & ESPEnum::HeadCircle;
	...
	if (pGroup->m_iESP & ESPEnum::KDR && pResource && !bLocal)
	{
		...
		if (iKills >= 20)
		{
			...
			if (iKDR >= 10)
				tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, std::format("KD^ [{}/{}]", iKills, iDeaths), Vars::Colors::IndicatorTextMid.Value, Vars::Menu::Theme::Background.Value);
		}
	}
	...
	if (pGroup->m_iESP & ESPEnum::Buffs)
	{
		...
		if (bCrits)
			...
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, "MINI-CRIT", Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
		...
		if (pPlayer->InCond(TF_COND_OFFENSEBUFF))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, "BUFF", Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_DEFENSEBUFF))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, "DEFENSE", Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
		if (pPlayer->InCond(TF_COND_REGENONDAMAGEBUFF))
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, "REGEN", Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value);
	...
	if (pGroup->m_iESP & ESPEnum::Flags)
	{
		if (pPlayer->m_bFeignDeathReady())
			...
			tCache.m_vText.emplace_back(ALIGN_TOPRIGHT, "FAKE DEATH", Vars::Menu::Theme::Active.Value, Vars::Menu::Theme::Background.Value);
...
void CESP::DrawPlayers()
{
	if (m_mPlayerCache.empty())
		return;
	const auto& fontsmall = H::Fonts.GetFont(FONT_ESP_SMALL);
	const int nTallSmall = fontsmall.m_nTall + H::Draw.Scale(2);
	...
	for (auto& [pEntity, tCache] : m_mPlayerCache)
	{
		...
		if (!GetDrawBounds(pEntity, x, y, w, h))
			continue;
		int l = x - H::Draw.Scale(3), r = x + w + H::Draw.Scale(6), m = x + w / 2;
		...
		if (tCache.m_bBox)
		{
			H::Draw.DrawCorneredBox(x + 1, y + 1, w - 2, h - 2, { 0, 0, 0, 255 }, { 0, 0, 0, 255 });
			H::Draw.DrawCorneredBox(x, y, w, h, tCache.m_tColor, { 0, 0, 0, 255 });
		}
		...
		for (auto& [iMode, flPercent, tColor, tOverfill, bAdjust] : tCache.m_vBars)
		{
			...
			{
					H::Draw.FillRect(x, y, w, h, { 0, 0, 0, 100 });
					H::Draw.LineRect(x - H::Draw.Scale(1), y - H::Draw.Scale(1), w + H::Draw.Scale(2), h + H::Draw.Scale(2), { 0, 0, 0, 255 });
				if (flPercent > 1.f)
				{
		...
		for (auto& [iMode, sText, tColor, tOutline] : tCache.m_vText)
		{
			...
			{
			case ALIGN_TOP:
				H::Draw.StringOutlined(fFont, m, t - tOffset, tColor, {0,0,0, 240}, ALIGN_BOTTOM, sText.c_str());
				tOffset += nTall;
				break;
			case ALIGN_BOTTOM:
			{
				std::string upperText = sText;
			...
			case ALIGN_LEFT:
				H::Draw.StringOutlined(fontsmall, l - lOffset, y - H::Draw.Scale(2) + h - h * std::min(tCache.m_flHealth, 1.f), tColor, tOutline, ALIGN_TOPRIGHT, sText.c_str());
				break;
			case ALIGN_TOPRIGHT:
				H::Draw.StringOutlined(fontsmall, r, y - H::Draw.Scale(2) + rOffset, tColor, tOutline, ALIGN_TOPLEFT, sText.c_str());
				rOffset += nTallSmall;
				break;
			case ALIGN_BOTTOMRIGHT:
				H::Draw.StringOutlined(fontsmall, r, y + h, tColor, tOutline, ALIGN_TOPLEFT, sText.c_str());
				break;
			}
		}
...
void CESP::DrawBones(CTFPlayer* pPlayer, matrix3x4* aBones, std::vector<int> vBones, Color_t tColor)
{
	for (size_t n = 1; n < vBones.size(); n++)
	{
		...
		auto HeadPos = pPlayer->GetHitboxCenter(aBones, HITBOX_HEAD);
		...
		if (SDK::W2S(HeadPos, vScreen1) && SDK::W2S(vBone2, vScreen2))
		{
			H::Draw.FillCircle(vScreen1.x, vScreen1.y, H::Draw.Scale(8), 40, tColor);
			H::Draw.LineCircle(vScreen1.x + 1, vScreen1.y + 1, H::Draw.Scale(8), 40, { 0, 0, 0, 255 });
		}
	}
}
