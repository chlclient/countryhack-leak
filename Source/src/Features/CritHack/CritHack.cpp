...
void CCritHack::Draw(CTFPlayer* pLocal)
{
	...
	{
		H::Draw.StringOutlined(fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value, align, "X");
		...
		H::Draw.StringOutlined(fFont, x, y += nTall, Vars::Colors::IndicatorTextMid.Value, Vars::Menu::Theme::Background.Value, align, std::format("{} DAMAGE REQUIRED", ceilf(m_flDamageTilFlip)).c_str());
	}
	...
	if (!m_bCritBanned)
	{
		if (m_iPotentialCrits > 0)
		{
			...
			H::Draw.StringOutlined(fFont, x, y += nTall, iCrits > 0 ? Vars::Colors::IndicatorTextGood.Value : Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value, align, std::format("{}{}/{} CRITS", iCrits, iCrits == BUCKET_ATTEMPTS ? "+" : "", m_iPotentialCrits).c_str());
		}
		else
			H::Draw.StringOutlined(fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value, Vars::Menu::Theme::Background.Value, align, "0/0 CRITS");
	}
