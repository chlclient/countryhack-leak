...
bool CAimbotProjectile::RunNeckbreaker(const Vec3& vPoint, float flPitch, float flYaw, int iSimTime, uint8_t iType, uint8_t iFlags, Vec3& vOutAngles)
{
	const int iAngleCount = 10; // Check 10 different roll angles
	const float flAngleStep = 360.f / static_cast<float>(iAngleCount);

	Vec3 vForward, vRight, vUp;
	CGameTrace wallTrace = {};
	CTraceFilterWorldAndPropsOnly wallFilter = {};

	for (int i = 0; i < iAngleCount; i++)
	{
		const float flRoll = static_cast<float>(i) * flAngleStep;
		const Vec3 vNeckbreakerAngles{ flPitch, flYaw, flRoll };

		// Quick wall obstruction check before expensive TestAngle
		Math::AngleVectors(vNeckbreakerAngles, &vForward, &vRight, &vUp);

		const Vec3 vMuzzle = m_tInfo.m_vLocalEye +
			vForward * m_tInfo.m_vOffset.x +
			vRight * m_tInfo.m_vOffset.y +
			vUp * m_tInfo.m_vOffset.z;

		// Ray trace is much cheaper than hull trace and is fine for "wall between muzzle and point"
		SDK::Trace(vMuzzle, vPoint, MASK_SHOT, &wallFilter, &wallTrace);

		if (wallTrace.DidHit() && wallTrace.fraction < 0.98f)
			continue;

		if (!m_tInfo.m_pProjectile
			? TestAngle(vPoint, vNeckbreakerAngles, iSimTime, iType, iFlags, false)
			: [&]() -> bool {
				Vec3 vTempAngles = vNeckbreakerAngles;
				return TestAngle(m_tInfo.m_pProjectile, vPoint, vTempAngles, iSimTime, iType, iFlags);
			}())
		{
			vOutAngles = vNeckbreakerAngles;
			return true;
		}
	}
...
bool CAimbotProjectile::HandlePoint(const Vec3& vOrigin, int iSimTime, float flPitch, float flYaw, float flTime, const Vec3& vPoint, uint8_t iType, uint8_t iFlags)
{
	...
	else if (!m_iResult && !m_tInfo.m_pProjectile)
	{
		...
		// Neckbreaker: try roll angles when main shot is blocked (silent aim only).
		// Different rolls shift the muzzle offset inside ProjSim.GetInfo / GetProjectileFireSetup,
		// producing slightly different spawn positions that can clear geometry.
		if (!bReturn && Vars::Aimbot::Projectile::Neckbreak.Value
			&& Vars::Aimbot::General::AimType.Value == Vars::Aimbot::General::AimTypeEnum::Silent)
		{
			if (RunNeckbreaker(vPoint, flPitch, flYaw, iSimTime, iType, iFlags, vAngles))
			{
				bReturn = m_iResult = true;
				m_flTimeTo = flTime + m_tInfo.m_flLatency;
			}
...
bool CAimbotProjectile::Aim(const Vec3& vCurAngle, const Vec3& vToAngle, Vec3& vOut, int iMethod)
{
	...
	*/
	...
	// Neckbreaker: preserve the roll component set during target solving
	if (Vars::Aimbot::Projectile::Neckbreak.Value && vToAngle.z != 0.f)
		vOut.z = vToAngle.z;
...
bool CAimbotProjectile::RunMain(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	const int nWeaponID = pWeapon->GetWeaponID();
	
	 {
		...
		const bool bPDown = GetAsyncKeyState('P') & 0x8000;

		if (Vars::Aimbot::Projectile::Airstrikesavepost.Value && nWeaponID == TF_WEAPON_PIPEBOMBLAUNCHER)
		{
			...
			CGameTrace tr = {};
			CTraceFilterWorldAndPropsOnly filter = {};
			SDK::Trace(vEye, vEye + vForward * 8192.f, MASK_SHOT | CONTENTS_GRATE, &filter, &tr);

			G::SavedPos = tr.endpos;
			G::HasSavedPos = true;
		}
		bPWasDown = bPDown;

		if (Vars::Aimbot::Projectile::Airstrikeclearpos.Value)
			G::HasSavedPos = false;

		if (Vars::Aimbot::General::AimType.Value > 0 && G::HasSavedPos)
		{
			Vec3 vAngle = Math::CalcAngle(pLocal->GetShootPos(), G::SavedPos);
