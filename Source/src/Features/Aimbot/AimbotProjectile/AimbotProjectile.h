...
private:
	...
	bool RunNeckbreaker(const Vec3& vPoint, float flPitch, float flYaw, int iSimTime, uint8_t iType, uint8_t iFlags, Vec3& vOutAngles);
...
//	bool CanSee(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, const Vec3& vFrom, const Vec3& vTo, const Target_t& target, float flTargetTime);
