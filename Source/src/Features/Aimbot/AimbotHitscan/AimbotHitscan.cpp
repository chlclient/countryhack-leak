...
static inline std::vector<Target_t> GetTargets(CTFPlayer* pLocal, CTFWeaponBase* pWeapon)
{
	...
	const bool bHeal = (nWeaponID == TF_WEAPON_MEDIGUN);

	// ========================================================================
	// PLAYERS (with lag records)
	// ========================================================================
	{
		...
		{
			eGroup = !F::AimbotGlobal.FriendlyFire() || (Vars::Aimbot::General::Ignore.Value & Vars::Aimbot::General::IgnoreEnum::Team)
				? EntityEnum::PlayerEnemy
				: EntityEnum::PlayerAll;

			if ((Vars::Aimbot::Hitscan::Modifiers.Value & Vars::Aimbot::Hitscan::ModifiersEnum::ExtinguishTeam) &&
				!F::AimbotGlobal.FriendlyFire() &&
				SDK::AttribHookValue(0, "jarate_duration", pWeapon) > 0)
			{
		...
		for (auto pEntity : H::Entities.GetGroup(eGroup))
		{
			...
			if (bTeam)
			{
				...
				if (!F::AimbotGlobal.FriendlyFire() &&
					SDK::AttribHookValue(0, "jarate_duration", pWeapon) > 0)
				{
			...
			for (TickRecord* pRecord : vRecords)
			{
				if (!pRecord)
					continue;

				Vec3 vPos = pRecord->m_vOrigin;
				vPos.z += pPlayer->GetOffset().z * 0.5f;

				float flFOVTo = 0.f;
				Vec3 vAngleTo{};

				if (!F::AimbotGlobal.PlayerBoneInFOV(pPlayer, vLocalPos, vLocalAngles, flFOVTo, vPos, vAngleTo, Vars::Aimbot::Hitscan::Hitboxes.Value))
					continue;

				const float flDistTo = vLocalPos.DistToSqr(vPos);
				...
				// Create target (priority is passed to the constructor like your original code)
				Target_t tTarget(pEntity, TargetEnum::Player, vPos, vAngleTo, flFOVTo, flDistTo, iPriority);
				tTarget.m_pRecord = pRecord;
				tTarget.m_bBacktrack = true;

				vTargets.emplace_back(std::move(tTarget));
			}
		}
	...
	if (Vars::Aimbot::General::Target.Value & Vars::Aimbot::General::TargetEnum::Building)
	{
		for (auto pEntity : H::Entities.GetGroup(EntityEnum::BuildingEnemy))
		{
			...
			vTargets.emplace_back(
				pEntity,
				pEntity->IsSentrygun() ? TargetEnum::Sentry :
				pEntity->IsDispenser() ? TargetEnum::Dispenser : TargetEnum::Teleporter,
				vPos, vAngleTo, flFOVTo, flDistTo
			);
		}
	}

	// ========================================================================
	// STICKIES (only touched + ignored on snipers)
	// ========================================================================
	if (Vars::Aimbot::General::Target.Value & Vars::Aimbot::General::TargetEnum::Stickies)
	{
			...
			nWeaponID != TF_WEAPON_SNIPERRIFLE_CLASSIC)
		{
			for (auto pEntity : H::Entities.GetGroup(EntityEnum::WorldProjectile))
			{
				...
				auto pSticky = pEntity->As<CTFGrenadePipebombProjectile>(); // adjust if needed
				if (!pSticky || !pSticky->m_bTouched())
					continue;
	...
	// ========================================================================
	// SORTING (only by FOV – no m_iPriority member exists)
	// ========================================================================
...
std::vector<HitscanHitbox_t> CAimbotHitscan::GetHitboxes(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CBaseEntity* pTarget)
{
	...
	if (auto pSet = pTarget->As<CBaseAnimating>()->GetHitboxSet())
	{
		...
		vBones.reserve(pSet->numhitboxes);
		...
		std::sort(vBones.begin(), vBones.end(), [](const auto& a, const auto& b)
		{
...
int CAimbotHitscan::GetHitboxPriority(int nHitbox, CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CBaseEntity* pTarget)
{
	...
	case TF_WEAPON_SNIPERRIFLE_CLASSIC:
	{
		...
		if (G::CanHeadshot
			|| (pLocal->InCond(TF_COND_AIMING) && (pSniperRifle->GetRifleType() == RIFLE_JARATE && SDK::AttribHookValue(0, "jarate_duration", pWeapon) > 0
	...
	// Force body aim conditions (ported + cleaned from raytrace)
	bool bForceBody = false;
	...
	if (Vars::Aimbot::Hitscan::Hitboxes.Value & Vars::Aimbot::Hitscan::HitboxesEnum::BodyaimIfLethal && bHeadshot)
	{
		...
		if (pPlayer)
		{
			...
			{
				auto pSniperRifle = pWeapon->As<CTFSniperRifle>();
				const int iDamage = ceilf(std::max(pSniperRifle->m_flChargedDamage(), 50.f) * pSniperRifle->GetBodyshotMult(pPlayer));
			...
			case TF_WEAPON_REVOLVER:
			{
				if (SDK::AttribHookValue(0, "set_weapon_mode", pWeapon) == 1)
				{
					const float flDist = pTarget->m_vecOrigin().DistTo(pLocal->m_vecOrigin());
					const float flMult = SDK::AttribHookValue(1, "mult_dmg", pWeapon);
					const int iDamage = ceilf(Math::RemapVal(flDist, 90.f, 900.f, 60.f, 21.f) * flMult);
					if (pPlayer->m_iHealth() <= iDamage)
						bForceBody = true;
				}
				break;
	...
	// Additional force-body conditions (from raytrace)
	if (nWeaponID == TF_WEAPON_SNIPERRIFLE || nWeaponID == TF_WEAPON_SNIPERRIFLE_DECAP || nWeaponID == TF_WEAPON_SNIPERRIFLE_CLASSIC)
	{
		auto pPlayer = pTarget->As<CTFPlayer>();
		if (pPlayer)
		{
			// Crit-immune / defense buff / vaccinator
			if (pPlayer->InCond(TF_COND_DEFENSEBUFF_HIGH) || pPlayer->InCond(TF_COND_DEFENSEBUFF)) // adapt IsBulletCharge if needed
				bForceBody = true;

			// Not scoped + no charge → force body (silent)
			if (!pLocal->InCond(TF_COND_ZOOMED) && pWeapon->As<CTFSniperRifle>()->m_flChargedDamage() <= 0.f)
				bForceBody = true;
		}
	}
	...
	const bool bHeadOnly = bHeadshot && (Vars::Aimbot::Hitscan::Hitboxes.Value & Vars::Aimbot::Hitscan::HitboxesEnum::HeadshotOnly);

	const int iHeadPriority = bHeadshot ? 0 : 1;
	const int iBodyPriority = bHeadOnly ? -1 : (bHeadshot ? 1 : 0);
	const int iMiscPriority = bHeadOnly ? -1 : 2;
	const int iLimbPriority = bHeadOnly ? -1 : 3;
...
std::vector<Vec3> CAimbotHitscan::GetHitboxPoints(CBaseEntity* pTarget, CTFWeaponBase* pWeapon, const Vec3& vMins, const Vec3& vMaxs, int nHitbox)
{
	...
	const bool bPlayer = pTarget->IsPlayer();
	...
	// Early out for smooth/assist when strength is 0
	...
	const float flScale = !bPlayer ? 0.5f : (Vars::Aimbot::Hitscan::MultipointScale.Value / 100.f);
	const Vec3 vHalf = (vMaxs - vMins) * 0.5f * flScale;

	// Center + 8 corners + 4 top/bottom mid-edges (same as you have)
	vPoints = {
		{},
...
int CAimbotHitscan::CanHit(Target_t& tTarget, CTFPlayer* pLocal, CTFWeaponBase* pWeapon)
{
	...
	Vec3* pHoldAngle = F::Ticks.GetShootAngle();
	if (pHoldAngle && bPlayer && vRecords.size() > 1)
	{
	...
	// Improved resolver integration
	float flResolverYaw = 0.f;
	if (bPlayer && F::Resolver.GetAngles(tTarget.m_pEntity->As<CTFPlayer>(), &flResolverYaw, nullptr))
	{
		// Resolver yaw is already applied in GetAngles
	}
	...
	for (auto pRecord : vRecords)
	{
		...
		for (auto& tHitbox : vHitboxes)
		{
			...
			{
				Math::VectorTransform(vPoint, *pTransform, vOrigin);
				vOrigin += vOffset;

				// ===== Head hitbox height correction (ported & adapted) =====
				if (tHitbox.m_iHitbox != -1 && tTarget.m_pEntity->GetHitboxToBase(tHitbox.m_iHitbox) == HITBOX_HEAD)
				{
					Vec3 vAdjusted = vOrigin;
					const float flStep = 0.75f;
					const int iMaxSteps = 14;

					for (int i = 0; i < iMaxSteps; i++)
					{
						if (SDK::VisPos(pLocal, tTarget.m_pEntity, m_vEyePos, vAdjusted))
						{
							// Optional: if you have a full trace that returns hitbox/hitgroup, you can tighten this further:
							// if (tr.hitbox == head && tr.hitgroup == HITGROUP_HEAD) { ... }
							vOrigin = vAdjusted;
							break;
						}
						vAdjusted.z += flStep;
				...
				Vec3 vAngles;
				bool bChanged = Aim(G::CurrentUserCmd->viewangles, Math::CalcAngle(m_vEyePos, vOrigin), vAngles);

				if (pHoldAngle)
					vAngles -= F::NoSpread.GetOffset();
				...
				// Improved visibility and hitbox checks
				bool bCanHit = false;
				if (!bChanged)
				{
					// If we didn't change angles, verify visibility
					if (SDK::VisPos(pLocal, tTarget.m_pEntity, m_vEyePos, vOrigin))
						bCanHit = true;
				}
				else if (Math::RayToOBB(m_vEyePos, vForward, vCheckMins, vCheckMaxs, *pTransform, flModelScale))
				{
					// Ray hits the hitbox
					Vec3 vTraceEnd = m_vEyePos + vForward * m_vEyePos.DistTo(vOrigin);
					if (SDK::VisPos(pLocal, tTarget.m_pEntity, m_vEyePos, vTraceEnd))
					{
						bCanHit = true;
					}
				}

				// Additional hull check for player targets
				if (bCanHit && pHullTransform)
				{
					if (!Math::RayToOBB(m_vEyePos, vForward, vHullMins, vHullMaxs, *pHullTransform))
						bCanHit = false;
				}
				...
				{
					// Fallback: can aim at it but ray doesn't hit
	...
	if (G::Attacking == 1 && nWeaponID != TF_WEAPON_LASER_POINTER)
	{
		...
		if (bLine || bBoxes)
		{
			...
			if (bLine)
			{
				...
				Vec3 vForward;
				Math::AngleVectors(tTarget.m_vAngleTo + pLocal->m_vecPunchAngle(), &vForward);
...
void CAimbotHitscan::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	// ===== DT Lock-on logic =====
	const bool bIsShifting = F::Ticks.m_bDoubletap; // Adjust this condition to whatever your DT shift detection is
	// Alternative common checks:
	// const bool bIsShifting = F::Ticks.IsShifting();
	// const bool bIsShifting = F::Ticks.m_bDoubletap || F::Ticks.m_iShiftedTicks > 0;

	if (!bIsShifting)
		m_pDTLockTarget = nullptr;
	// ============================
	...
	// Prefer locked target if we are shifting
	if (m_pDTLockTarget)
	{
		std::sort(vTargets.begin(), vTargets.end(), [&](const Target_t& a, const Target_t& b)
			{
				if (a.m_pEntity == m_pDTLockTarget) return true;
				if (b.m_pEntity == m_pDTLockTarget) return false;
	...
	{
		// Strict lock while shifting
		if (m_pDTLockTarget && tTarget.m_pEntity != m_pDTLockTarget)
			continue;
		...
		if (G::Attacking == 1 && nWeaponID != TF_WEAPON_LASER_POINTER)
		{
			...
			// Lock the target while we are in a DT shift
			if (bIsShifting)
				m_pDTLockTarget = tTarget.m_pEntity;
		}
