...
void CMenu::DrawMenu()
{
	...
	bool bWantOpen = m_bIsOpen;

	const float flFadeSpeed = 4.5f;
	const float flDelta = I::GlobalVars->frametime * flFadeSpeed;
	...
	if (Begin("Main", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground))
	{
		...
		if (CurrentBind != DEFAULT_BIND)
		{
			...
			pDrawList->AddRectFilled(vDrawPos, vDrawPos + ImVec2(flSize - flInset, flOffset - flInset), F::Render.Background0, H::Draw.Scale(0));
			pDrawList->AddRectFilled(vDrawPos + ImVec2(0, flOffset - flInset), vDrawPos + ImVec2(flSize - flInset, flOffset), F::Render.Background2);
			pDrawList->AddRectFilled(vDrawPos + ImVec2(0, flOffset), vDrawPos + ImVec2(flSize - flInset, vWindowSize.y), F::Render.Background0, H::Draw.Scale(0));
			...
			pDrawList->AddRectFilled(vDrawPos, vDrawPos + ImVec2(flSize - flInset, vWindowSize.y), F::Render.Background0, H::Draw.Scale(0));
		...
		FTabs(
			{
				...
				{ "Exploits" },
				{ "Visuals", "ESP", "Main", "Menu" },
				{ "Misc##" },
				{ "Logs", "Playerlist", "Settings##", "Output" },
				{ "Settings", "Configs", "Binds", "Materials", "Misc" }
			},
			...
			{ { ICON_MD_GPS_NOT_FIXED }, { ICON_MD_ATTACH_MONEY }, { ICON_MD_PHOTO_LIBRARY }, { ICON_MD_ARTICLE }, { ICON_MD_IMPORT_CONTACTS }, { ICON_MD_MISCELLANEOUS_SERVICES } },
			{ H::Draw.Scale(10), 0 }, {},
			{}, { H::Draw.Scale(22), 0 }
		);
	...
	case 0:
	{
		if (BeginTable("AimbotTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				{
					FToggle(Vars::Aimbot::Projectile::Airstrikesavepost, FToggleEnum::Left);
					FToggle(Vars::Aimbot::Projectile::Airstrikeclearpos, FToggleEnum::Right);
					FToggle(Vars::Aimbot::Projectile::Neckbreak, FToggleEnum::Left); FTooltip("that one viral pasteris feature that had a long tooltip", Hovered);
	...
	case 1:
	{
		if (BeginTable("DrawTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Debug", 8))
				{
					FToggle(Vars::Misc::DebugDraw::NoSpreadDraw, FToggleEnum::Left);
					FToggle(Vars::Misc::DebugDraw::ResolverData, FToggleEnum::Right);
					FToggle(Vars::Misc::DebugDraw::PacketJitter, FToggleEnum::Left);
				} EndSection();
				if (Vars::Debug::Options.Value)
				{
					if (Section("##Debug"))
		...
		if (BeginTable("HvHTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("Antiaim", 10))
				{
					...
					PopTransparent();
					SetCursorPos({ GetWindowWidth() / 2 + GetStyle().WindowPadding.x / 2, GetRowPos() + H::Draw.Scale(16) });
					FToggle(Vars::AntiAim::MinWalk, FToggleEnum::Left);
					FToggle(Vars::AntiAim::FlipRealFake, FToggleEnum::Left);
				} EndSection();
				if (Vars::Debug::Options.Value)
				{
			...
			&& BeginTable("VisualsESPTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Chams"))
				{
					...
					{
						FMDropdown("Visible", &tGroup.m_tChams.Visible, FDropdownEnum::Left);
						FMDropdown("Occluded", &tGroup.m_tChams.Occluded, FDropdownEnum::Right);
					}
					else
		...
		if (BeginTable("MenuTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("Settings", 8))
				{
					...
					FColorPicker(Vars::Menu::Theme::Accent2, FColorPickerEnum::Right);

					FColorPicker(Vars::Menu::Theme::Background, FColorPickerEnum::Left);
					FColorPicker(Vars::Menu::Theme::Active, FColorPickerEnum::Right);
					FColorPicker(Vars::Menu::Theme::Inactive, FColorPickerEnum::Left);
			...
			PushDisabled(F::PlayerUtils.m_bLoad);
			{
				...
				if (FButton("Export", FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					...
					if (fStream.is_open())
					{
						...
						SDK::Output("Countryhack", "Copied playerlist to clipboard", DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
					}
				}

				{
					...
					if (FButton("Import", FButtonEnum::Fit | FButtonEnum::SameLine))
					{
						...
						{
							SDK::Output("Countryhack", "Failed to import playerlist", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
						}
					}
					...
					if (FBeginPopupModal("ImportPlayerlist"))
					{
						...
						if (FButton("Import", FButtonEnum::Left))
						{
							...
							F::PlayerUtils.m_bSave = true;
							SDK::Output("Countryhack", "Imported playerlist", DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
				...
				if (FButton("Backup", FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					try
					{
						...
						SDK::Output("Countryhack", "Saved backup playerlist", DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
					}
					catch (...)
					{
						SDK::Output("Countryhack", "Failed to backup playerlist", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG);
					}
				}
			}
			PopDisabled();
			...
			auto fDrawConfigs = [](std::string& sStaticName, bool bVisual = false)
				{
					...
					for (auto& [entry, sConfigName] : vConfigs)
					{
						...
						SetCursorPos({ GetWindowWidth() - H::Draw.Scale(iOffset += 25), vOriginalPos.y + H::Draw.Scale(9) });
						bool bSave = IconButton(ICON_MD_UPLOAD);
...
void CMenu::AddDraggable(const char* sLabel, ConfigVar<DragBox_t>& tVar, bool bShouldDraw, ImVec2 vSize)
{
	...
	PushStyleColor(ImGuiCol_Border, { 255, 255, 255, 255 });
...
void CMenu::DrawBinds()
{
	...
	if (Begin("Binds", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoSavedSettings))
	{
		...
		int i = 0; for (auto& [sName, sInfo, sState, iBind, tBind] : vInfo)
		{
			...
			FText(sInfo.c_str()); // kind (Hold/Toggle/Class/etc.)
			PopStyleColor();
			...
			PushStyleColor(ImGuiCol_Text, tBind.m_bActive ? F::Render.Accent.Value : F::Render.Inactive.Value);
			FText(sDisplayState.c_str()); // var state (On/Off/etc.)
			PopStyleColor();
...
void CMenu::DrawNotifications()
{
	...
	for (auto it = F::Notifications.m_vNotifications.begin(); it != F::Notifications.m_vNotifications.end();)
	{
		if (it->m_flCreateTime + it->m_flLifeTime + it->m_flPanTime < SDK::PlatFloatTime())
			it = F::Notifications.m_vNotifications.erase(it);
		else
			++it;
	}
	if (F::Notifications.m_vNotifications.empty())
		return;

	ImGuiIO& io = GetIO();
	ImDrawList* pDrawList = GetForegroundDrawList();

	float flH = H::Draw.Scale(40);
	float flY = !ShouldReverseY() ? H::Draw.Scale(8) : io.DisplaySize.y - H::Draw.Scale(8) - flH;
	float flRounding = H::Draw.Scale(0);

	for (auto& tNotification : F::Notifications.m_vNotifications)
	{
		PushFont(F::Render.FontRegular);
		float flTextW = FCalcTextSize(tNotification.m_sText.c_str()).x;
		PopFont();

		PushFont(F::Render.IconFont);
		float flIconW = FCalcTextSize(ICON_MD_INFO).x;
		PopFont();

		float flPadding = H::Draw.Scale(10);
		float flGap = H::Draw.Scale(6);
		float flW = flPadding + flIconW + flGap + flTextW + flPadding;
		float flX = !ShouldReverseX() ? H::Draw.Scale(8) : io.DisplaySize.x - H::Draw.Scale(8) - flW;
		...
		float flLifeFrac = Math::RemapVal(flCreate + flLife - flPan - flTime, 0.f, flLife, 0.f, 1.f);
		flX -= (flW + H::Draw.Scale(8)) * (1.f - flEaseX) * (!ShouldReverseX() ? 1.f : -1.f);

		Color_t tAccent = Vars::Menu::Theme::Accent.Value;
		Color_t tBackground = Vars::Menu::Theme::Background.Value;
		Color_t tActive = Vars::Menu::Theme::Active.Value;

		auto fToU32 = [](Color_t c) -> ImU32
			{
				return IM_COL32(c.r, c.g, c.b, c.a);
			};

		ImVec2 vMin = { flX + 1, flY + 1 };
		ImVec2 vMax = { flX + flW - 1, flY + flH - 1 };

		pDrawList->AddRectFilled(vMin, vMax, fToU32(tBackground), flRounding);

		float flBarY = flY + flH - 1 - H::Draw.Scale(2);
		float flBarH = H::Draw.Scale(2);
		ImVec2 vBarMin = { flX + 1, flBarY };
		ImVec2 vBarMax = { flX + flW - 1, flBarY + flBarH };

		pDrawList->PushClipRect(vBarMin, vBarMax, true);
		pDrawList->AddRectFilled(vMin, vMax, fToU32(tAccent.Alpha(50)), flRounding);
		pDrawList->PopClipRect();

		ImVec2 vFillMax = { flX + 1 + (flW - 2) * flLifeFrac, flBarY + flBarH };
		pDrawList->PushClipRect(vBarMin, vFillMax, true);
		pDrawList->AddRectFilled(vMin, vMax, fToU32(tAccent.Alpha(255)), flRounding);
		pDrawList->PopClipRect();

		pDrawList->AddRect({ flX, flY }, { flX + flW, flY + flH }, fToU32(tBackground.Lerp({ 127, 127, 127 }, 2.f / 9)), H::Draw.Scale(0));

		PushFont(F::Render.FontRegular);
		ImVec2 vTextSize = CalcTextSize(tNotification.m_sText.c_str());
		PopFont();

		PushFont(F::Render.IconFont);
		ImVec2 vIconSize = CalcTextSize(ICON_MD_INFO);
		PopFont();

		float flContentH = std::max(vTextSize.y, vIconSize.y);
		float flContentY = flY + (flH - flContentH) * 0.5f;

		ImVec2 vIconPos =
		{
			flX + flPadding,
			flContentY + (flContentH - vIconSize.y) * 0.5f
		};

		ImVec2 vTextPos =
		{
			flX + flPadding + flIconW + flGap,
			flContentY + (flContentH - vTextSize.y) * 0.5f
		};

		PushFont(F::Render.IconFont);
		pDrawList->AddText(vIconPos, fToU32(tAccent.Alpha(255)), ICON_MD_INFO);
		PopFont();

		PushFont(F::Render.FontRegular);
		pDrawList->AddText(
			{ vTextPos.x + 1, vTextPos.y + 1 },
			fToU32(tBackground.Alpha(255)),
			tNotification.m_sText.c_str()
		);

		pDrawList->AddText(
			vTextPos,
			fToU32(tActive.Alpha(255)),
			tNotification.m_sText.c_str()
		);
		PopFont();

		flY += (flH + H::Draw.Scale(8)) * flEaseY * (!ShouldReverseY() ? 1.f : -1.f);
	}
}
...
void CMenu::Render()
{
...

	{
		...
		const char* szText = "countryhack.xyz | v1.10 | Releasex64";
		ImVec2 textSize = ImGui::CalcTextSize(szText);

		ImFont* font = F::Render.FontLarge;
		float fontSize = ImGui::GetFontSize();
		if (font) {
			textSize = ImGui::CalcTextSize(szText);
		}

		const float padding = 3.0f;
		const float topBarHeight = 1.0f;

		ImVec2 pos(10, 10);

		ImVec2 bgMin = ImVec2(pos.x - padding, pos.y - padding);
		ImVec2 bgMax = ImVec2(pos.x + textSize.x + padding, pos.y + textSize.y + padding);

		draw->AddRectFilled(bgMin, bgMax, ImU32(F::Render.Background1));

		draw->AddRect(bgMin, bgMax, ImU32(F::Render.Accent));

		draw->AddRect(
			ImVec2(bgMin.x - 1, bgMin.y - 1),
			ImVec2(bgMax.x + 1, bgMax.y + 1),
			IM_COL32(0, 0, 0, 255),
			0.0f, 0, 1.0f
		);

		const ImU32 textColor = ImU32(F::Render.Active);

		draw->AddText(font, fontSize, pos, textColor, szText);
	...
	if (m_bIsOpen)
	{
		...
		AddDraggable("Features", Vars::Menu::IndicatorsDisplay, FGet(Vars::Menu::Indicators) & Vars::Menu::IndicatorsEnum::Features);
