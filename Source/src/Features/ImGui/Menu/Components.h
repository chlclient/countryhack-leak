...
namespace ImGui
{
	...
	inline void RenderTwoToneBackground(float flSize, ImU32 uTitle, ImU32 uBackground, bool bHorizontal = false)
	{
		...
		if (!bHorizontal)
		{
			pDrawList->AddRectFilled(vDrawPos, vDrawPos + ImVec2(vSize.x, flSize), uTitle, H::Draw.Scale(4), 0);
			pDrawList->AddRectFilled(vDrawPos + ImVec2(0, flSize), vDrawPos + vSize, uBackground, H::Draw.Scale(4), 0);
		}
		else
		{
			pDrawList->AddRectFilled(vDrawPos, vDrawPos + ImVec2(flSize, vSize.y), uTitle, H::Draw.Scale(4), 0);
			pDrawList->AddRectFilled(vDrawPos + ImVec2(flSize, 0), vDrawPos + vSize, uBackground, H::Draw.Scale(4), 0);
		}
	}
	inline void RenderTwoToneBackground(float flSize, ImU32 uTitle, ImU32 uBackground, ImU32 uBorder, float flInset = H::Draw.Scale(), bool bVertical = true, bool bTwoToneBorder = true)
	{
		...
		{
			if (!bTwoToneBorder)
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flInset), vDrawPos + ImVec2(vSize.x - flInset, flSize), uTitle, H::Draw.Scale(0), 0);
			else
			{
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flInset), vDrawPos + ImVec2(vSize.x - flInset, flSize - H::Draw.Scale()), uTitle, H::Draw.Scale(0), 0);
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flSize - H::Draw.Scale()), vDrawPos + ImVec2(vSize.x - flInset, flSize), uBorder);
			}
			pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flSize), vDrawPos + vSize - ImVec2(flInset, flInset), uBackground, H::Draw.Scale(0), 0);
		}
		else
		{
			if (!bTwoToneBorder)
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flInset), vDrawPos + ImVec2(flSize, vSize.y - flInset), uTitle, H::Draw.Scale(0), 0);
			else
			{
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flInset), vDrawPos + ImVec2(flSize - H::Draw.Scale(), vSize.y - flInset), uTitle, H::Draw.Scale(0), 0);
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flSize - H::Draw.Scale(), flInset), vDrawPos + ImVec2(flSize, vSize.y - flInset), uBorder);
			}
			pDrawList->AddRectFilled(vDrawPos + ImVec2(flSize, flInset), vDrawPos + vSize - ImVec2(flInset, flInset), uBackground, H::Draw.Scale(0), 0);
		}
		
		flInset += H::Draw.Scale(0.5f) - 0.5f - H::Draw.Scale();
		pDrawList->AddRect(vDrawPos + ImVec2(flInset, flInset), vDrawPos + vSize - ImVec2(flInset, flInset), F::Render.Black, H::Draw.Scale(0), ImDrawFlags_None, H::Draw.Scale());
	}
		...
		if (iSteps < 21 || (vPosMax.x - vPosMin.x) / iSteps > 6.f)
		{
			...
			if (!vSteps.empty())
			{
				for (size_t i = 0; i < vSteps.size(); i++)
				{
					...
					pDrawList->AddRectFilled(vPos + ImVec2(vSteps[i].first, vPosMin.y), vPos + ImVec2(vSteps[i].second, vPosMax.y), uPrimary);
				}
	...
	inline bool Section(const char* sLabel, float flPaddingMod = 0.f, float flMinHeight = 28.f, bool bForceHeight = false)
	{
		...
		{
			if (sLabel[0] != '#')
				RenderTwoToneBackground(H::Draw.Scale(20), F::Render.AccentDark, F::Render.Background0p5, F::Render.Black);
			else
				RenderBackground(F::Render.Background0p5, F::Render.Black);
		}
		...
		if (sLabel[0] != '#')
		{
			...
			std::string cleanLabel = StripDoubleHash(sLabel);
			ImVec2 textSize = CalcTextSize(cleanLabel.c_str());

			// Center the text horizontally
			float availWidth = GetColumnWidth();                    // or GetContentRegionAvail().x
			float textX = (availWidth - textSize.x) * 0.5f;
			ImVec2 originalPos = GetCursorPos();
			//SetCursorPosX(originalPos.x + textX);
			SetCursorPosY(originalPos.y - H::Draw.Scale(5));
			TextColored(F::Render.Active, cleanLabel.c_str());
			PopFont();

			// Restore Y position and add padding
			SetCursorPosY(originalPos.y);
			DebugDummy({ 0, H::Draw.Scale(9 + flPaddingMod) });
		}
	...
	{	// WOW fuck this
		...
		// Lerp state: keyed by window ID so each FTabs instance is independent
		static std::unordered_map<ImGuiID, float> sBarLerpY;
		static std::unordered_map<ImGuiID, bool> sBarLerpInit;
		ImGuiID barID = GetID("FTabsBar");
		...
		for (size_t i = bReverse ? vEntries.size() - 1 : 0; i < vEntries.size(); (bReverse ? i-- : i++))
		{
			for (size_t j = bReverse ? vEntries[i].size() - 1 : 0; j < vEntries[i].size(); (bReverse ? j-- : j++))
			{
				...
				else if (iTabState == 2)
				{
					...
					case 2: // right
					{
						float targetY = vDrawPos.y + vCurBarOffset.y + flBarSizeMod;
						float& lerpY = sBarLerpY[barID];
						if (!sBarLerpInit[barID]) { lerpY = targetY; sBarLerpInit[barID] = true; }
						lerpY = ImLerp(lerpY, targetY, ImGui::GetIO().DeltaTime * 12.f);

						float x0 = (iBar == 1)
							? vDrawPos.x + vCurBarOffset.x
							: vDrawPos.x + vCurBarOffset.x + vNewSize.x - H::Draw.Scale(flBarThickness);
						float x1 = (iBar == 1)
							? x0 + H::Draw.Scale(flBarThickness)
							: vDrawPos.x + vCurBarOffset.x + vNewSize.x;
						/*pDrawList->AddRectFilled(
							ImVec2(x0, lerpY),
							ImVec2(x1, lerpY + vNewSize.y - flBarSizeMod * 2),
							F::Render.Accent
						);*/
					...
					case 4: // bottom
					{
						float targetY = (iBar == 3)
							? vDrawPos.y + vCurBarOffset.y
							: vDrawPos.y + vCurBarOffset.y + vNewSize.y - H::Draw.Scale(flBarThickness);
						float& lerpY = sBarLerpY[barID];
						if (!sBarLerpInit[barID]) { lerpY = targetY; sBarLerpInit[barID] = true; }
						lerpY = ImLerp(lerpY, targetY, ImGui::GetIO().DeltaTime * 12.f);

						/*pDrawList->AddRectFilled(
							ImVec2(vDrawPos.x + vCurBarOffset.x + flBarSizeMod, lerpY),
							ImVec2(vDrawPos.x + vCurBarOffset.x + vNewSize.x - flBarSizeMod, lerpY + H::Draw.Scale(flBarThickness)),
							F::Render.Accent
						);*/
						break;
					}
	...
	inline bool FToggle(const char* sLabel, bool* pVar, int iFlags = FToggleEnum::None, bool* pHovered = nullptr)
	{
		...
		// Lerp alpha for the accent fill: keyed per label so each toggle is independent
		static std::unordered_map<ImGuiID, float> sToggleLerpAlpha;
		ImGuiID toggleID = GetID(sLabel);
		float& lerpAlpha = sToggleLerpAlpha[toggleID];
		float targetAlpha = *pVar ? 1.f : 0.f;
		lerpAlpha = ImLerp(lerpAlpha, targetAlpha, ImGui::GetIO().DeltaTime * 10.f);

		ImDrawList* pDrawList = GetWindowDrawList();
		float flBoxSize = H::Draw.Scale(10.f);

		// Determine if the checkbox should be on the far right of the button
		bool bCheckBoxRight = (iFlags & FToggleEnum::Right);

		// Calculate X position: 
		// If Right aligned, place it at the end of the button width minus padding and box size.
		// Otherwise, use your standard left padding.
		float flRelativeX = bCheckBoxRight ? (vSize.x - flBoxSize - H::Draw.Scale(8)) : H::Draw.Scale(8);

		ImVec2 vBoxPos = GetDrawPos() + ImVec2(vOriginalPos.x + flRelativeX, vOriginalPos.y + H::Draw.Scale(3 + 3 * iWraps));
		ImVec2 vMin = { vBoxPos.x, vBoxPos.y };
		ImVec2 vMax = { vBoxPos.x + flBoxSize, vBoxPos.y + flBoxSize };

		// Hover Effect (Moved after X calculation so the circle matches the box)
		if (IsItemHovered() && !Disabled)
		{
			ImColor tTransparent = tColor;
			tTransparent.Value.w *= (IsMouseDown(ImGuiMouseButton_Left) ? 0.1f : 0.05f) * GetStyle().Alpha;
			pDrawList->AddRectFilled(
				vBoxPos - ImVec2(H::Draw.Scale(10) - flBoxSize / 2, H::Draw.Scale(10) - flBoxSize / 2),
				vBoxPos + ImVec2(flBoxSize / 2 + H::Draw.Scale(10), flBoxSize / 2 + H::Draw.Scale(10)),
				tTransparent
			);
		}

		// 1. Draw Background (always)
		pDrawList->AddRectFilled(vMin, vMax, ImColor(30, 30, 30, 255));

		// 2. Draw Accent fill on top with lerped alpha
		if (lerpAlpha > 0.001f)
		{
			ImColor tAccent = iFlags & FToggleEnum::PlainColor ? F::Render.Active : F::Render.Accent;
			tAccent.Value.w *= lerpAlpha;
			pDrawList->AddRectFilled(vMin, vMax, tAccent);
		}

		// 3. Draw Outline
		pDrawList->AddRect(vMin, vMax, ImColor(0, 0, 0, 255), 0.0f, 0, 1.f);

		// Text Rendering
		for (size_t i = 0; i < iWraps; i++)
		{
			// If the box is on the right, the text should stay on the left (Scale 4).
			// If the box is on the left, the text needs to be pushed right (Scale 24).
			float flTextX = bCheckBoxRight ? H::Draw.Scale(4) : H::Draw.Scale(24);

			SetCursorPos({ vOriginalPos.x + flTextX, vOriginalPos.y + H::Draw.Scale(5 + 18 * i) });
	...
	inline bool FDropdown(const char* sLabel, int* pVar, std::vector<const char*> vEntries, std::vector<int> vValues = {}, int iFlags = FDropdownEnum::None, int iSizeOffset = 0, const char* sDefaultPreview = "None", bool* pHovered = nullptr, int* pModified = nullptr)
	{
		...
		// Further decreased height
		ImVec2 vSize = { GetWindowWidth(), H::Draw.Scale(bTitle ? 32 : 16) };
		...
		PushStyleVar(ImGuiStyleVar_FramePadding, { 0.f, H::Draw.Scale(bTitle ? 9.5f : 3.0f) });
		PushItemWidth(vSize.x);

		// Black background for main dropdown
		ImDrawList* pDrawList = GetWindowDrawList();
		ImVec2 vMainMin = GetCursorScreenPos();
		ImVec2 vMainMax = vMainMin + vSize;
		//pDrawList->AddRectFilled(vMainMin, vMainMax, IM_COL32(0, 0, 0, 255), 0.0f);
		pDrawList->AddRect(vMainMin, vMainMax, IM_COL32(0, 0, 0, 255), 0.0f, 0, H::Draw.Scale(1.0f));
		...
		if (bActive)
		{
			// Black background for popup
			ImRect popupRect = GetCurrentWindow()->ClipRect;
			//GetWindowDrawList()->AddRectFilled(popupRect.Min, popupRect.Max, IM_COL32(0, 0, 0, 255));
			//pDrawList->AddRect(vMainMin, vMainMax, IM_COL32(0, 0, 0, 255), 0.0f, 0, H::Draw.Scale(1.0f));
			...
			int i = 0; for (auto& sEntry : vEntries)
			{
				...
				if (iFlags & FDropdownEnum::Multi)
				{
					...
					// Multi-select but styled like normal single-select
					...
					SetCursorPos(vOriginalPos2 + ImVec2(H::Draw.Scale(18), 0));  // less indent now that no checkbox
		...
		if (BeginComboPreview())
		{
			...
			if (bTitle)
			{
				SetCursorPos(vOriginalPos2 + ImVec2(H::Draw.Scale(12), H::Draw.Scale(-5)));   // adjusted
				...
				SetCursorPos(vOriginalPos2 + ImVec2(H::Draw.Scale(12), H::Draw.Scale(8)));    // adjusted
				TextUnformatted(TruncateText(sPreview, vSize.x - H::Draw.Scale(45)).c_str());

				SetCursorPos(vOriginalPos2 + ImVec2(vSize.x - H::Draw.Scale(24), H::Draw.Scale(-2)));
				IconImage(bActive ? ICON_MD_KEYBOARD_ARROW_UP : ICON_MD_KEYBOARD_ARROW_DOWN);
			}
			else
			{
				SetCursorPos(vOriginalPos2 + ImVec2(H::Draw.Scale(12), H::Draw.Scale(-1)));
				TextUnformatted(TruncateText(sPreview, vSize.x - H::Draw.Scale(45)).c_str());
