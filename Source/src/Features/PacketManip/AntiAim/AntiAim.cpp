...
bool CAntiAim::ShouldRun(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	if (!pLocal->IsAlive() || pLocal->IsAGhost() || pLocal->IsTaunting() || pLocal->m_MoveType() != MOVETYPE_WALK || pLocal->InCond(TF_COND_HALLOWEEN_KART)
		|| G::Attacking == 1 || F::AutoRocketJump.IsRunning() || F::Ticks.m_bDoubletap
...
static inline int GetEdge(CTFPlayer* pEntity, const float flYaw)
{
	...
	return flEdgeLeftDist > flEdgeRightDist ? -1 : 1; // -1 = wall on right, 1 = wall on left
}
...
void CAntiAim::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd, bool bSendPacket)
{
	...
	const bool bFlip = Vars::AntiAim::FlipRealFake.Value;

	Vec2 vAngReal = {};
	Vec2 vAngFake = {};

	vAngReal.x = iAntiBackstab != 2 ? GetPitch(pCmd->viewangles.x) : pCmd->viewangles.x;
	vAngReal.y = !iAntiBackstab ? GetYaw(pLocal, pCmd, bFlip) : pCmd->viewangles.y;

	vAngFake.x = iAntiBackstab != 2 ? GetPitch(pCmd->viewangles.x) : pCmd->viewangles.x;
	vAngFake.y = !iAntiBackstab ? GetYaw(pLocal, pCmd, !bFlip) : pCmd->viewangles.y;

	if (F::AntiCheatCompatibility.Active())
	{
		Math::ClampAngles(vAngReal);
		Math::ClampAngles(vAngFake);
	}

	vRealAngles = vAngReal;
	vFakeAngles = vAngFake;

	const Vec2& vAngles = bSendPacket ? vFakeAngles : vRealAngles;
