...
static inline void HandleMovement(CTFPlayer* pPlayer, MoveData* pLastRecord, MoveData& tCurRecord, std::deque<MoveData>& vRecords)
{
    ...
    {
        // Reset history if the player suddenly hits a wall (steep slope)
        CGameTrace trace = {};
        CTraceFilterWorldAndPropsOnly filter = {};
        filter.pSkip = pPlayer;

        SDK::TraceHull(pLastRecord->m_vOrigin, pLastRecord->m_vOrigin + pLastRecord->m_vVelocity * TICK_INTERVAL,
                       pPlayer->m_vecMins() + PLAYER_ORIGIN_COMPRESSION,
                       pPlayer->m_vecMaxs() - PLAYER_ORIGIN_COMPRESSION,
                       pPlayer->SolidMask(), &filter, &trace);
...
void CMovementSimulation::Store()
{
    ...
    for (auto pEntity : H::Entities.GetGroup(EntityEnum::PlayerAll))
    {
        ...
        {
            float flNewAvg = vSimTimes.front() * 0.7f + flDeltaTime * 0.3f;
            vSimTimes.front() = flNewAvg;
        }
...
static inline void VisualizeRecords(MoveData& tRecord1, MoveData& tRecord2, Color_t tColor, float flStraightFuzzyValue)
{
    ...
    const bool bStraight = fabsf(flYaw) * tRecord1.m_vVelocity.Length2D() * iTicks < flStraightFuzzyValue;
...
static inline bool GetYawDifference(MoveData& tRecord1, MoveData& tRecord2, bool bStart, float* pYaw, float flStraightFuzzyValue, int iMaxChanges = 0, int iMaxChangeTime = 0, float flMaxSpeed = 0.f)
{
    ...
    *pYaw = Math::NormalizeAngle(flYaw1 - flYaw2);
    if (flMaxSpeed && tRecord1.m_iMode != MoveEnum::Air)
        *pYaw *= std::max(0.f, std::min(tRecord1.m_vVelocity.Length2D() / flMaxSpeed, 1.f));
...
void CMovementSimulation::GetAverageYaw(MoveStorage& tMoveStorage, int iSamples)
{
    ...
    // Weighted average: give more influence to recent records (already done by the loop order)
...
bool CMovementSimulation::StrafePrediction(MoveStorage& tMoveStorage, bool bStrafe, bool bHitchance)
{
    if (bStrafe && Vars::Aimbot::Projectile::StrafePrediction.Value & (tMoveStorage.m_bDirectMove ? Vars::Aimbot::Projectile::StrafePredictionEnum::Ground : Vars::Aimbot::Projectile::StrafePredictionEnum::Air))
    {
        ...
        this->GetAverageYaw(tMoveStorage, iStrafeSamples);

        // hitchance check based on consistency of yaw changes
        if (bHitchance && !tMoveStorage.m_pPlayer->m_vecVelocity().IsZero() && Vars::Aimbot::Projectile::HitChance.Value)
        {
            ...
            for (size_t i = 0; i < iSamples; i++)
            {
                ...
                if (tMoveStorage.m_MoveData.m_flMaxSpeed)
                    ...
                    //flYaw *= std::max(0.f, std::min(pRecord1.m_vVelocity.Length2D() / tMoveStorage.m_MoveData.m_flMaxSpeed, 1.f));
...
void CMovementSimulation::RunTick(MoveStorage& tMoveStorage, bool bPath, RunTickCallback* pCallback)
{
    ...
    if (tMoveStorage.m_flAverageYaw)
    {
        ...
        if (!tMoveStorage.m_bDirectMove && !tMoveStorage.m_pPlayer->InCond(TF_COND_SHIELD_CHARGE))
        {
            ...
            if (Vars::Aimbot::Projectile::MovesimFrictionFlags.Value & Vars::Aimbot::Projectile::MovesimFrictionFlagsEnum::RunReduce)
                flMult = GetFrictionScale(tMoveStorage.m_MoveData.m_vecVelocity.Length2D(), tMoveStorage.m_flAverageYaw, tMoveStorage.m_MoveData.m_vecVelocity.z + SDK::GetGravity() * TICK_INTERVAL, 0.f, 56.f);
        }
    ...
    // ---- IMPROVEMENT: Validate new origin is inside world ----
    Vec3 vNewOrigin = tMoveStorage.m_MoveData.m_vecAbsOrigin;
    CGameTrace trace = {};
    CTraceFilterWorldAndPropsOnly filter = {};
    filter.pSkip = tMoveStorage.m_pPlayer;
    SDK::TraceHull(vNewOrigin, vNewOrigin,
                   tMoveStorage.m_pPlayer->m_vecMins() + PLAYER_ORIGIN_COMPRESSION,
                   tMoveStorage.m_pPlayer->m_vecMaxs() - PLAYER_ORIGIN_COMPRESSION,
                   tMoveStorage.m_pPlayer->SolidMask(), &filter, &trace);
    if (trace.DidHit())
    {
        // Revert to previous origin (simulation failed or stuck)
        tMoveStorage.m_MoveData.m_vecAbsOrigin = tMoveStorage.m_vPath.back();
        tMoveStorage.m_bFailed = true;
        RestoreBounds(tMoveStorage.m_pPlayer);
        return;
    }
...
float CMovementSimulation::GetPredictedDelta(CBaseEntity* pEntity)
{
    ...
    if (!vSimTimes.empty())
    {
        ...
        case 1: // max
            return *std::max_element(vSimTimes.begin(), vSimTimes.end());
        }
    }
